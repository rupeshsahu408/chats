...
- **Forgot password (`ForgotPasswordPage`, route `/forgot-password`):** Recovery-key based reset — no email link, no OTP. Steps: username → recovery key (paste 12 words OR upload a `.txt`/`.json` file the helper greps for a valid BIP-39 window) → strong new password → success. The phrase never leaves the device: `auth.beginPasswordReset` issues a 5-minute HMAC-bound nonce (always, even for unknown usernames, to prevent enumeration); the client derives the Ed25519 keypair from the phrase (`deriveIdentityFromPhrase`), signs the nonce, and posts the signature + claimed pubkey + new password to `auth.completePasswordReset`. The server compares the claimed pubkey to the stored `users.identity_pubkey` and runs `ed25519.verify` before bcrypting the new password (cost 12). All failure modes collapse to one generic "couldn't verify your recovery key" error. Per-IP and per-username rate limits via `lib/rateLimit`. Linked from the password sub-step on `RandomLoginPage`. Server: `lib/passwordReset.ts` (HMAC nonce store mirroring `loginRisk.ts`), procedures in `auth.ts`. **No DB schema changes** — uses the public key already stored at signup.
