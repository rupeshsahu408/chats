...
- **Single active session login (Apr 2026).** WhatsApp-style "only one device at a time" enforcement. New `loginConflicts` table (status + requester IP/device/city/country + HMAC continuation-nonce hash + 5-min TTL) and `securityAlerts` table (`kind: rejected_login_attempt | account_secured`, jsonb payload). New `users.requirePasswordChange` boolean flips the next sign-in into a forced password reset.
  - Server (`apps/server/src/lib/sessionConflicts.ts` + `auth.ts` V3 endpoints): `beginLoginV3` is the new login entry — after password OK it returns `{ok | challenge | deviceConflict | mustChangePassword}`. A `deviceConflict` parks the new device with a continuation nonce and pushes `login_request_pending` over WS to all of the user's existing sockets. `pollLoginConflict` is the new device's wait loop (2.5s tick); `completeAfterApproval` atomically issues the new session, deletes every other session for that user, and broadcasts `session_revoked` with `reason: "single_active"`. Existing-device side: `listPendingLoginRequests` + `resolveLoginRequest({accept|reject})` handle the prompt and emit `login_request_resolved` plus a `security_alert` row on reject. `acknowledgeSecurityAlert({dismiss|secure})` — the `secure` action sets `requirePasswordChange`, deletes all sessions, and pushes `session_revoked` with `reason: "secured"`. `submitNewPasswordAfterSecure` redeems a 15-min HMAC token to set the new password and issue a fresh session. `checkSessionStatus` is the public liveness probe used by the client to detect force-logout. Same-device re-login (matching device fingerprint + IP prefix) silently replaces the old session without prompting.
  - Client: `RandomLoginPage` calls `beginLoginV3` and gained 3 new steps — `deviceConflict` (waiting screen with cancel + 2.5s polling that auto-claims via `completeAfterApproval`), `mustChangePassword` (new password form), and `passwordSuggestion` (skippable post-replacement password refresh). Global `SessionGuard` (mounted in `App.tsx` next to `SessionSync`) renders `IncomingLoginRequestModal` (existing-device "is this you?" prompt with countdown + accept/reject) and `SecurityAlertModal` ("It was me" / "Secure my account"). `sessionEventsStore` is a tiny zustand counter store that bridges WS pushes from `SessionSync.tsx` to the guard, which re-fetches via `listPendingLoginRequests` / `listSecurityAlerts` on every tick (and polls every 20s as a WS fallback). On `session_revoked`, the guard hits `checkSessionStatus`; if our session row is gone, it clears auth + unlocked identity, shows a calm "signed out from another device" toast, and bounces to `/login`.
  - WS: 4 new events on `WsServerEventSchema` — `login_request_pending`, `login_request_resolved`, `security_alert`, `session_revoked`. Auth router's `housekeeping` cron now also expires stale pending conflicts so polling new devices get a clean `expired` answer instead of waiting forever. The legacy `beginLoginV2` / `completeLoginV2` endpoints stay in place for back-compat but are no longer called from the client.
