...
- **Client login (`RandomLoginPage`):** username + password, **plus a "Sign in with a passkey" button** that uses discoverable credentials (no username required). If a derived identity already exists in IndexedDB for the returned userId, either path skips straight in. Otherwise the recovery-key step still runs so we can re-derive Ed25519/X25519 on a fresh device — passkeys handle authentication, the recovery phrase remains the source of truth for the E2E key material.
