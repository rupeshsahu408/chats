...
- **Client signup (`RandomIdSignupPage`):** 7 steps — username (debounced availability) → password + confirm → slide-to-verify puzzle → recovery key with **mandatory file download** before proceeding → optional display name → optional bio → optional avatar (resized client-side to 256×256 JPEG ≤60KB via `lib/avatar.ts`) → welcome popup. Account is created right after the recovery key is downloaded; profile fields are flushed via `me.updateProfile` after the photo step.
