...
- **Client signup (`RandomIdSignupPage`):** 10 steps — username (debounced availability) → password + confirm → daily-verification password → slide-to-verify puzzle → key-ceremony animation → **Recovery Kit PDF** (mandatory before proceeding) → **Passkey** (skippable, "Recommended" — `PasskeySetupCard`) → optional display name → optional bio → optional avatar (resized client-side to 256×256 JPEG ≤60KB via `lib/avatar.ts`) → welcome popup. Account is created right after the recovery kit is downloaded; profile fields are flushed via `me.updateProfile` after the photo step.
...
- **Passkeys / WebAuthn (`@simplewebauthn/server` + `@simplewebauthn/browser`):** New `passkeys` table (id, userId fk, credentialId unique, publicKey bytea, counter, transports, deviceName, isBackedUp, createdAt, lastUsedAt). Server router `trpc/routers/passkey.ts` exposes `getRegistrationOptions` / `verifyRegistration` / `list` / `rename` / `delete` (protected) + `getAuthenticationOptions` / `verifyAuthentication` (public; mints session inline using existing JWT helpers). RP ID + origin are derived per request from the `Origin` header so the same backend works on localhost, replit.dev and production. Challenges live in an in-memory single-use TTL store (`lib/passkeyChallenge.ts`, 60 s, mirroring `botChallenge.ts`). Client lib (`lib/passkey.ts`) wraps `startRegistration` / `startAuthentication`, surfaces capability checks (`isPasskeySupported`, `isPlatformAuthenticatorAvailable`), suggests a UA-based device label, and friendly-formats WebAuthn errors. Visual `components/PasskeySetupCard.tsx` renders a "Recommended" green card with fingerprint glyph, used both in the signup passkey step and inside the Settings Passkeys modal. `SettingsPage → Security → Passkeys` lists registered passkeys with inline rename and delete confirmation.
- **Client login (`RandomLoginPage`):** username + password, **plus a "Sign in with a passkey" button** that uses discoverable credentials (no username required). If a derived identity already exists in IndexedDB for the returned userId, the app's E2EE state is unlocked in-place during sign-in. If no local identity exists (brand-new device), the user is still signed in but stays in the locked-decryption state — **the recovery-phrase entry has been removed from the login flow itself**; restoring keys onto a new device now happens through a separate dedicated path (Settings → Recovery, or the unlock gate), not as an inline step on the login screen.
- **Forgot password (`ForgotPasswordPage`, route `/forgot-password`):** Recovery-key based reset — no email link, no OTP. Steps: username → recovery key (paste 12 words OR upload a `.pdf`/`.txt`/`.json` file — the actual Veil Recovery Kit PDF is supported via `pdfjs-dist`, lazy-loaded into its own ~134KB-gzipped chunk so it never bloats the main bundle; the helper greps the extracted text for a valid BIP-39 window) → strong new password (with confirm + match validation) → success → optional **passkey upgrade** ("Enable now" or "Skip"). "Enable now" sets a `veil:pending_passkey_setup` localStorage hint and clears the snooze, so the post-login screen surfaces the `PasskeySetupCard` immediately. The phrase never leaves the device: `auth.beginPasswordReset` issues a 5-minute HMAC-bound nonce (always, even for unknown usernames, to prevent enumeration); the client derives the Ed25519 keypair from the phrase (`deriveIdentityFromPhrase`), signs the nonce, and posts the signature + claimed pubkey + new password to `auth.completePasswordReset`. The server compares the claimed pubkey to the stored `users.identity_pubkey` and runs `ed25519.verify` before bcrypting the new password (cost 12). All failure modes collapse to one generic `"Invalid recovery key. Please try again."` error. Per-IP and per-username rate limits via `lib/rateLimit`. Linked from both the password sub-step on `RandomLoginPage` **and** a "Forgot your password?" link on the `LoginPage` method picker. Server: `lib/passwordReset.ts` (HMAC nonce store mirroring `loginRisk.ts`), procedures in `auth.ts`. **No DB schema changes** — uses the public key already stored at signup.
...
- **Single active session login — "new device decides" model (Apr 2026).** WhatsApp-style "only one device at a time" enforcement, but the *new* device makes the call (no waiting on the old one to approve). The previous "old-device-approves" flow (with `loginConflicts` DB table, polling, and `IncomingLoginRequestModal`) has been removed. `securityAlerts` table (`kind: rejected_login_attempt | account_secured`, jsonb payload) and `users.requirePasswordChange` boolean are kept exactly as before.
  - Server (`apps/server/src/lib/sessionConflicts.ts` + `auth.ts` V3 endpoints): `beginLoginV3` is the login entry — after password OK it returns `{ok | challenge | deviceConflict | mustChangePassword}`. On `deviceConflict` it mints a stateless 5-min HMAC `pendingLoginToken` (carrying `userId`, requester fingerprint, IP prefix, device label, city, country) and returns it together with the existing-device descriptors (`existingDevice/City/Country/LastUsedAt` + `activeSessionCount`) — no DB row, no WS push to the old device at this point. The new device then calls one of two endpoints: `confirmReplaceSession({pendingLoginToken})` verifies the token, issues a fresh session, atomically deletes every other session for the user, and broadcasts `session_revoked` with `reason: "replaced_by_new_device"` so any still-connected old device tears down its UI immediately; `rejectLoginAttempt({pendingLoginToken})` verifies the token, inserts a `rejected_login_attempt` row in `securityAlerts`, and pushes `security_alert` to the existing device(s). `acknowledgeSecurityAlert({dismiss|secure})` — the `secure` action sets `requirePasswordChange`, deletes all sessions, and pushes `session_revoked` with `reason: "secured"`. `submitNewPasswordAfterSecure` redeems a 15-min HMAC token. `checkSessionStatus` is the liveness probe. Same-device re-login (matching device fingerprint + IP prefix) silently replaces the old session without prompting.
  - Client: `RandomLoginPage` calls `beginLoginV3` and now has a Yes/No `deviceConflict` screen ("@user is currently signed in on iPhone in Paris · last used 2 hr ago — was that you?") with two buttons. YES → `confirmReplaceSession` → `landSession` → straight to `welcome` (no post-login password prompt). NO → `rejectLoginAttempt` → toast + back to the password step. The previous polling effect, `passwordSuggestion` step, and `IncomingLoginRequestModal` are gone. Global `SessionGuard` (mounted in `App.tsx`) now only renders `SecurityAlertModal` (existing-device "It was me" / "Secure my account" prompt). `sessionEventsStore` is a small zustand counter store that bridges WS pushes from `SessionSync.tsx` to the guard, which re-fetches via `listSecurityAlerts` on every alert tick (20s polling fallback). On `session_revoked`, the guard hits `checkSessionStatus`; if our session row is gone it clears local auth + unlocked identity and shows a tailored toast ("Signed out — you (or someone) just signed in on another device" for `replaced_by_new_device`, "Signed out — this account was secured" for `secured`).
  - WS: `WsServerEventSchema` now has only 2 single-active-session events — `security_alert` and `session_revoked` (with `reason: "replaced_by_new_device" | "secured" | "manual"`). The legacy `beginLoginV2` / `completeLoginV2` endpoints stay in place for back-compat but are no longer called from the client.
